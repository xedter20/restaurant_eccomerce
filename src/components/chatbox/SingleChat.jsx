...
const ENDPOINT = process.env.REACT_APP_API_URL; // If you are deploying the app, replace the value with "https://YOUR_DEPLOYED_APPLICATION_URL" then run "npm run build" to create a production build
...
const SingleChat = ({ fetchAgain, setFetchAgain }) => {
  ...
  useEffect(() => {
    socket.on('message recieved', newMessageRecieved => {
      if (
        !selectedChatCompare ||
        selectedChatCompare._id !== newMessageRecieved.chat[0]
      ) {
        console.log('here');
        ...
        if (!messages.find(m => m._id === newMessageRecieved._id)) {
  ...
  const sendMessage = async e => {
    ...
    if (e.key === 'Enter' && newMessage) {
      ...
      try {
        ...
        setMessages([...messages, response.data]); // Add new message with existing messages
      } catch (error) {
        return toast({
          title: 'Error Occured!',
        ...
        <>
          <Text
            ...
            {!selectedChat.isGroupChat ? (
              <>
                {loggedUser && getSender(user, selectedChat.users)}
